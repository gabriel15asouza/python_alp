...
def remove_iguais(nums):
    ...
    for x in sorted_nums:
        index = sorted_nums.index(x)
        if x in sorted_nums[index+1:]:
...
def cripto(frase):
    ...
    for pal in lista:
        ...
        lista_let = list(sorted_pal)
        for x in lista_let:
            index = lista_let.index(x)
            if x in lista_let[index+1:]:
                lista_let.remove(x)
        final = ''.join(lista_let)
        resp.append(final)
        sentence = " ".join(resp)
    return sentence
...
def soma(n1, n2):
    ...
    resto = 0
    for x in n1:
        soma = x + n2[cont] + resto
        if soma < 10:
            resp.append(soma)
            resto = 0
        elif soma > 10 and cont == len(n1)-1:
            resp.append(soma-10)
            resp.append(resto)
        else:
            resp.append(soma-10)
