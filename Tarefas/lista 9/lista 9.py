...
def first_last6(nums):
    ...
    for x in nums:
        ...
        txt.append(x)
    txt = ''.join(txt)
    return txt.find('6') == 0 or txt.find('6')== len(txt)-1
...
def middle_way(a, b):
    lista = []
    lista.append(a[len(a)//2])
    lista.append(b[len(a)//2])
    return lista
...
def pego_correndo(speed, is_birthday):
    if speed <= 60 or (is_birthday == True and speed <= 65):
        return 0
    elif 60<speed<=80 or (is_birthday == True and speed <=85):
        return 1
    return 2
...
def alarm_clock(day, vacation):
    fds = [0,6]
    week = range(1,6)
    if vacation == True and day in fds:
        return 'off'
    elif vacation == True and day in week:
        return '10:00'
    elif day in fds:
        return '10:00'
    elif day in week:
